...
import MBCSGroupProber from "./mbcsgroupprober.js";
import SBCSGroupProber from "./sbcsgroupprober.js";
import Latin1Prober from "./latin1prober.js";
import EscCharSetProber from "./escprober.js";
...
const supportedEncodingsDenormalized = (function () {
    ...
    for (const encoding of supportedEncodings) {
        denormalizedEncodings.push(encoding.toLocaleLowerCase(), encoding.toLocaleLowerCase().replace(/-/g, ""));
    }
...
function UniversalDetector(options) {
    ...
    this.close = function () {
        ...
        if (this._mInputState == _state.highbyte) {
            for (var i = 0, prober; prober = this._mCharsetProbers[i]; i++) {
                if (!prober || !prober.getCharsetName() || !canDetectEncoding(prober.getCharsetName()))
                    continue;
                this.results.push({
                    "encoding": prober.getCharsetName(),
                    "confidence": prober.getConfidence()
